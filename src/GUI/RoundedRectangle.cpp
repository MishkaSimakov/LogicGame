...
void RoundedRectangle::generateShape()
{
    ...
    if (m_corners_to_round & TOP_RIGHT)
    {
        for(i = 0; i < m_points; ++i)
        {
            ...
            m_shape.setPoint(point_id, sf::Vector2f(x + m_position.x + m_size.x - m_radius, m_position.y - y + m_radius));
            ++point_id;
        }
    }
    else
    ...
    if (m_corners_to_round & BOTTOM_RIGHT)
    {
        ...
        for(i = 0; i < m_points; ++i)
        {
            ...
            m_shape.setPoint(point_id, sf::Vector2f(m_position.x + m_size.x + x - m_radius, m_position.y + m_size.y - m_radius + y));
            ++point_id;
        }
    }
    else
    ...
    if (m_corners_to_round & BOTTOM_LEFT)
    {
        ...
        for(i = 0; i < m_points; ++i)
        {
            x += m_radius / m_points;
            y = std::sqrt(m_radius * m_radius - x * x);
            m_shape.setPoint(point_id, sf::Vector2f(m_position.x + m_radius - x, m_position.y + m_size.y - m_radius + y));
            ++point_id;
        }
    }
    else
